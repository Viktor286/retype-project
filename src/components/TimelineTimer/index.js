...
function TimelineTimer({ totalChars, userName }) {
  ...
  const averageTimeLimitMs = useMemo(
    () => Math.floor(totalChars / statRatios.averageCps) * 1000,
    [totalChars],
  );
  ...
  return (
    <section className="timer">
      <div className="timeline">
        ...
        <div className="player-time-bar" style={{ width: keysCompletedPercent + '%' }}>
          &nbsp;
        </div>
        <div
          className="best-time-bar"
          style={{ width: `${bestTimeProgress >= 100 ? 100 : bestTimeProgress}%` }}
        >
          &nbsp;
        </div>
